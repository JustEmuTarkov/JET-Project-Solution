...
{
    class OnDeadSpeaker : GenericPatch<OnDeadSpeaker>
    {
        public OnDeadSpeaker() : base(transpiler: nameof(PatchTranspile)) { }

        protected override MethodBase GetTargetMethod() => typeof(Player)
            .GetMethod("OnDead", Constants.DefaultBindingFlags);
        ...
        static IEnumerable<CodeInstruction> PatchTranspile(ILGenerator generator, IEnumerable<CodeInstruction> instructions)
        {
                ...
                if (codes[i].opcode == OpCodes.Callvirt)
                {
                    switch (callVirtCount)
                    {
                        ...
                        case 2:
                            ...
                            break;
                        default:
                            Debug.LogError("[OnDeadPatch] Mismatched callvirt instruction count.");
                            break;
                    }
            ...
            {
                JET.Utility.Logger.Patch.LogPatchErrorWithMessage(MethodBase.GetCurrentMethod(), "Could not callvirt instructions.");
                return instructions;
            }
